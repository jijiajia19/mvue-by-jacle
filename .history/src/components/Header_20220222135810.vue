<template>
    <div class="container">
        <div class="topbar-nav">
            <ul class="nav-wrap">
                <li v-for="item in navs" class="nav clearfix" :key="item.name">
                    <template v-if="item.name === '小米商城'">
                        <a :href="item.sourceUrl" class="nav-name">{{
                            item.name
                        }}</a>
                    </template>
                    <template v-else>
                    ...
                    <span class="nav-separate" v-show="$index != 8"
                        >|{{ $index }}</span
                    >
                </li>
            </ul>
    ...
    data() {
        return {
            ...
            navs: [
                ...
                {
                    name: '小米网移动版',
                    sourceUrl: 'http://www.mi.com/c/appdownload/',
                },
                {
                    name: '问题反馈',
                    sourceUrl: 'http://static.mi.com/feedback/',
                },
                {
                    name: 'Select Region',
                    sourceUrl: 'http://www.mi.com/index.html',
                },
            ],
        };
    },
